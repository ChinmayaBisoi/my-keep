...
import { useKeepDispatch } from "../../pages/my-keep";
...
const Note = ({
  ...
  const deleteNote = () => {
    dispatch({ type: "delete-note", createdAt: note.createdAt });
  };
    ...
    <div className="flex rounded-8 border border-[#525355] hover:border-[#626365] hover:shadow-26 hover:shadow-[#000]">
      ...
      <div className="flex flex-col justify-between items-center py-4 pr-4">
        <div
          onClick={() => {
            dispatch({
              type: "add-or-remove-pin-from-existing-note",
        ...
        >
          ...
          {isHoveringPin && (
            <div
              className={`absolute top-30 md:block hidden rounded-4 border z-2 border-[#696969] bg-[#000000] w-80 px-8 py-2 text-center text-[#f1f1f1] text-12`}
            >
        ...
        <div
          ...
          className="cursor-pointer w-28 h-28 min-w-28 rounded-full p-8 hover:bg-[#3a3a3a] ml-8 flex items-center justify-center"
        >
